...
This part is pretty simpler and was easy to set up. The next step was figuring out how to parse the raw binary from a rom file and turn that into instructions for the CPU. The CHIP-8 has a strange opcode definition in that each opcode if a 16 bit/2 byte value that contains the instruction and the data for that instruction. This would be simple enough were it not for the fact that each apart from the first four bits, each following [nibble](https://en.wikipedia.org/wiki/Nibble) can be an identifier for the instruction or data for it depending on the instruction it maps to For instance, the opcode **0x00E0** means *clear the screen* but opcode **0x1234** means *jump to address 0x234.*
